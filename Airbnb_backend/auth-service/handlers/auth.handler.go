...
func (ac *AuthHandler) Login(ctx *gin.Context) {
	...
	if !userVerif.Verified {
		ac.logger.Info("Please verify your email")
	...
	if err := utils.VerifyPassword(user.Password, credentials.Password); err != nil {
		ac.logger.Error("Invalid password")
	...
	ac.logger.Infof("Login saccessful: %s", credentials)
...
func (ac *AuthHandler) Registration(ctx *gin.Context) {
	...
	if existingUser != nil {
		ac.logger.Errorf("Username alrady exists")
	...
	if existingUser1 != nil {
		ac.logger.Errorf("Email alrady exists")
	...
	if !utils.ValidatePassword(user.Password) {
		ac.logger.Errorf("Invalid password format: %s", codes.Error)
	...
	if err != nil {
		if strings.Contains(err.Error(), "email already exist") {
			ac.logger.Error("Email already exists")
			span.SetStatus(codes.Error, err.Error())
			ctx.JSON(http.StatusConflict, gin.H{"status": "error", "message": err.Error()})
			return
		}
		ac.logger.Errorf("Error: %s", err.Error())
		span.SetStatus(codes.Error, err.Error())
		ctx.JSON(http.StatusBadGateway, gin.H{"status": "error", "message": err.Error()})
		return
	}
	ac.logger.Info("Registration successful")
...
func (ac *AuthHandler) VerifyEmail(ctx *gin.Context) {
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			...
			ac.logger.Errorf("Invalid verification code or user doesn't exist: %s", updatedUser.VerificationCode)
			...
			ac.logger.Errorf("Error during verification: %v", err)
			ctx.JSON(http.StatusInternalServerError, gin.H{"status": "fail", "message": "Internal Server Error"})
		}
		return
	}
	if updatedUser.VerifyAt.Before(time.Now()) {
		ac.logger.Error("The verify token has expired ")
	...
	if updatedUser.Verified {
		...
		ac.logger.Errorf("User already verified: %s", updatedUser.Email)
	...
	if err != nil {
		...
		ac.logger.Errorf("Error updating user record: %v", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"status": "fail", "message": "Internal Server Error"})
		return
	}

	span.SetStatus(codes.Ok, "Email verified successfully")
	ac.logger.Infof("Email verified successfully for user: %s", updatedUser.Email)
...
func (ac *AuthHandler) ForgotPassword(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&payload); err != nil {
		ac.logger.Errorf("fail: %v", err.Error())
		span.SetStatus(codes.Error, err.Error())
		ctx.JSON(http.StatusBadRequest, gin.H{"status": "fail", "message": err.Error()})
		return
	}
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			ac.logger.Errorf("Invalid email: %v", codes.Error)
			span.SetStatus(codes.Error, "Invalid email")
			ctx.JSON(http.StatusBadRequest, gin.H{"status": "fail", "message": "Invalid email"})
		} else {
	...
	if !user.Verified {
		ac.logger.Error("Account not verified")
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			ac.logger.Error("User dosnt exist")
	...
	if err := ac.authService.SendPasswordResetToken(updatedUser, spanCtx); err != nil {
		...
		ac.logger.Errorf("Error sending password reset email: %v", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"status": "fail", "message": "Error sending password reset email"})
		return
	}
	ac.logger.Info("You will receive a reset email.")
...
func (ac *AuthHandler) ResetPassword(ctx *gin.Context) {
	...
	if payload.Password != payload.PasswordConfirm {
		ac.logger.Error("Password do not match")
	...
	if err != nil {
		if err == mongo.ErrNoDocuments {
			ac.logger.Error("The reset token is invalid ")
	...
	if updatedUser.PasswordResetAt.Before(time.Now()) {
		ac.logger.Error("The reset token has expired ")
	...
	ac.logger.Info("Password data updated saccessfully")
